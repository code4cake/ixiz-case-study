...
export default function App() {
  ...
  if (view.name === 'events') {
    return <EventList onOpenEvent={(eventId: string) => setView({ name: 'eventDetail', eventId })} />;
  }
  ...
  if (view.name === 'guestReserve') {
    return (
      <GuestReserve eventId={view.eventId} token={view.token} onConfirmed={() => setView({ name: 'guestConfirmed' })} />
    );
  }
